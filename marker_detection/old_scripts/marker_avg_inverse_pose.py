...
cap = cv2.VideoCapture(0, cv2.CAP_V4L2)  # 4 for webcam external )Adjust the device index as necessary
...
while True:
    ...
    if len(inverse_poses) > 2:
        ...
        publish_marker(-1, avg_tvec, (0, 1, 1), is_detected=True, orientation=orientation)  # Blue for camera pose
    else: # remove the blue marker if there are less than 3 markers detected
        publish_marker(-1, avg_tvec, (0, 0, 1), is_detected=False)

    # display marker bounding boxes and poses
    for detection in detections:
        corners = np.int32(detection.corners)
        cv2.polylines(frame, [corners], isClosed=True, color=(0, 255, 0), thickness=2)
        cv2.putText(frame, f"ID {detection.tag_id}", (corners[0][0], corners[0][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
