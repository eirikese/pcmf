...
detected_marker_ids = []
inverse_poses = []
avg_rvec = np.zeros((3, 1))
avg_tvec = np.zeros((3, 1))
...
def invert_pose(rvec, tvec_ros):
    """Inverts the pose to get the camera pose from the marker pose."""
...
def average_poses(poses):
    """Averages multiple poses to estimate a single pose."""
...
def publish_marker(id, position, color, is_detected=True, orientation=None):
    """Publishes a visualization marker in ROS."""
    marker = Marker()
...
def image_callback(msg):
    """Callback function to process each frame from ROS image topic."""
    try:
    ...
    for detection in detections:
        ...
        if marker_id in known_marker_positions:
            ...
            if success:
                ...
                publish_marker(marker_id, known_marker_positions[marker_id], (0, 1, 0))

    # Initialize avg_tvec here to handle cases when fewer than 3 markers are detected
    avg_tvec = np.zeros((3, 1))
    avg_rvec = np.zeros((3, 1))  # similarly define avg_rvec to prevent other potential issues

    for marker_id, position in known_marker_positions.items():
        if marker_id not in detected_marker_ids:
            publish_marker(marker_id, position, (1, 0, 0))
    ...
    if len(inverse_poses) > 2:
        ...
        publish_marker(-1, avg_tvec, (0, 1, 1), is_detected=True, orientation=orientation)
        print("detected markers: ", detected_marker_ids)
    else:
