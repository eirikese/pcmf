...
public:
    ...
    void mapPointsCallback(const sensor_msgs::PointCloud2ConstPtr& msg) {
        ...
        // // Publish the reconstructed surface as a PolygonMesh
        // pcl_msgs::PolygonMesh polygon_mesh_msg;
        // pcl_conversions::fromPCL(reconstructed_mesh, polygon_mesh_msg);
        // polygon_mesh_msg.header.frame_id = msg->header.frame_id; // Use the same frame as the input
        // polygon_mesh_msg.header.stamp = ros::Time::now();
        // reconstructed_surface_pub_.publish(polygon_mesh_msg);
